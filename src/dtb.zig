...
pub const Prop = union(enum) {
    ...
    pub fn format(prop: Prop, writer: *std.Io.Writer) !void {
        switch (prop) {
            .TimebaseFrequency => |v| try writer.print("timebase-frequency: <0x{x:0>2}>", .{v}),
            .RiscvIsa => |v| try writer.print("riscv,isa: {s}", .{v}),
            .RiscvIsaBase => |v| try writer.print("riscv,isa-base: {s}", .{v}),
            .RiscvIsaExtensions => |v| try (StringListFormatter{ .string_list = v }).write("riscv,isa-extensions: ", writer),
            .RiscvNdev => |v| try writer.print("riscv,ndev: <0x{x:0>2}>", .{v}),
            .Method => |v| try writer.print("method: {s}", .{v}),
            .EnableMethod => |v| try writer.print("enable-method: {s}", .{v}),
            .MmuType => |v| try writer.print("mmu-type: {s}", .{v}),
            .Model  => |v| try writer.print("model: {s}", .{v}),
            .StdoutPath => |v| try writer.print("stdout-path: {s}", .{v}),
            .DmaCoherent => try writer.print("dma-coherent", .{}),
